...
if __name__ == "__main__":
    ...
    for target_data, target_label in target_test_loader:
        feature = model.calculate_global_classifier_input_vector(target_data).detach().numpy()
        label = target_label.numpy().reshape((-1, 1))
    ...
    df_lr_input = pd.DataFrame(data=classifier_data, columns=col_names)
