...
class FederatedTargetLearner(object):
    ...
    def __init__(self, wrapper, target_train_loader, target_val_loader, patience=200, max_global_epochs=500):
    ...
    def _check_exists(self):
        if self.wrapper.check_discriminator_exists() is False:
            raise RuntimeError('Discriminator not set.')

    def _change_to_train_mode(self):
        # for wrapper in self.wrapper_list:
        self.wrapper.change_to_train_mode()

    def _change_to_eval_mode(self):
        # for wrapper in self.wrapper_list:
        self.wrapper.change_to_eval_mode()

    def save_model(self, task_id=None, timestamp=None):
        """Save trained model."""
        self.wrapper.save_model(self.root, task_id, self.task_meta_file_name, timestamp=timestamp)
    ...
    def train_target_models(self, epochs, optimizer, lr, train_bottom_only=False, curr_global_epoch=0, global_epochs=1):
        ...
        for ep in range(epochs):
            ...
            for batch_idx, (batch_data, batch_label) in enumerate(self.target_train_loader):
                ...
                class_loss = self.wrapper.compute_classification_loss(batch_data, batch_label)
                ...
                with torch.no_grad():
                    if (batch_idx + 1) % 5 == 0:
                        ...
                        tgt_cls_acc, tgt_cls_auc, tgt_cls_ks = test_classification(self.wrapper, self.target_val_loader)
                        ...
                        if score > self.best_score:
                            ...
                            param_dict = self.wrapper.get_global_classifier_parameters()
                            metric_dict = dict()
    ...
    def apply_lr_constraint(self):
        cls_parameters = self.wrapper.get_global_classifier_parameters(get_tensor=True)
    ...
    def train_target_with_alternating(self, global_epochs, top_epochs, bottom_epochs,
        ...
        if dann_exp_result:
            ...
            optimizer = optim.SGD(self.wrapper.parameters(), lr=lr, momentum=0.90)
        else:
            ...
            optimizer = optim.SGD(self.wrapper.parameters(), lr=lr, momentum=0.99, weight_decay=0.0001)
            # optimizer = optim.SGD(self.wrapper.parameters(), lr=lr, momentum=0.9, weight_decay=0.00001)
            # optimizer = optim.Adam(self.wrapper.parameters(), lr=lr, weight_decay=0.0001)
        curr_lr = lr
        ...
        self.wrapper.freeze_bottom(is_freeze=True)
        for ep in range(global_epochs):

            print(f"[INFO] ===> global epoch {ep}, start fine-tuning top")
            self.wrapper.freeze_top(is_freeze=False)
            self.wrapper.freeze_bottom(is_freeze=True)
            # self.wrapper.freeze_top(is_freeze=True)
            # self.wrapper.freeze_bottom(is_freeze=False)
            # self.wrapper.freeze_bottom(is_freeze=True, region_idx_list=self.fine_tuning_region_idx_list)
            ...
            # self.wrapper.freeze_bottom_aggregators(is_freeze=False, region_idx_list=self.fine_tuning_region_idx_list)
            # self.wrapper.freeze_bottom_aggregators(is_freeze=False)
    ...
    def train_target_as_whole(self, global_epochs, lr, task_id, dann_exp_result=None):
        ...
        if dann_exp_result:
            ...
            self.get_weight_constraints(dann_exp_result)
            optimizer = optim.SGD(self.wrapper.parameters(), lr=lr, momentum=0.9)
            # optimizer = optim.Adam(self.wrapper.parameters(), lr=lr)
        else:
            print("[INFO] Do not apply DANN lr parameters")
            optimizer = optim.SGD(self.wrapper.parameters(), lr=lr, momentum=0.9, weight_decay=0.01)
